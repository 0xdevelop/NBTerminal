...
func TestTerminalContextMenuReflectsSelectionAndRoutesCommands(t *testing.T) {
	...
	cleared := 0
	found := 0
	items := terminalContextMenuItems(terminal, uikit.ContextMenuState{}, func() { found++ }, func() { cleared++ })
	if len(items) != 5 {
		t.Fatalf("context menu item count = %d, want 5", len(items))
	}
	...
	if items[4].Title != "Clear Terminal	Ctrl+Shift+L" {
		t.Fatalf("clear item title = %q", items[4].Title)
	}
	items[4].Callback()
	if cleared != 1 {
	...
	items = terminalContextMenuItems(terminal, uikit.ContextMenuState{HasSelection: true}, func() {}, func() {})
