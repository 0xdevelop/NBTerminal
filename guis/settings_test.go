...
func TestValidateSettingsDraft(t *testing.T) {
	valid := settingsDraft{Language: "zh-CN", CommandTimeoutSeconds: 45, TerminalFontSize: 16}
	if err := valid.Validate(); err != nil {
		t.Fatalf("valid settings rejected: %v", err)
	}
	for _, draft := range []settingsDraft{
		{Language: "unsupported", CommandTimeoutSeconds: 45, TerminalFontSize: 16},
		{Language: "en", CommandTimeoutSeconds: 0, TerminalFontSize: 16},
		{Language: "en", CommandTimeoutSeconds: 86401, TerminalFontSize: 16},
		{Language: "en", CommandTimeoutSeconds: 45, TerminalFontSize: config.TerminalFontSizeMin - 1},
		{Language: "en", CommandTimeoutSeconds: 45, TerminalFontSize: config.TerminalFontSizeMax + 1},
	} {
...
func TestPersistSettingsDraftCommitsAndReloads(t *testing.T) {
	...
	config.GlobalConfig = &config.FileConfig{Language: "en", Terminal: &config.TerminalSettings{CommandTimeoutSeconds: 60, FontSize: 14}}
	...
	if config.GlobalConfig.Language != "zh-CN" || config.GlobalConfig.Terminal.CommandTimeoutSeconds != 17 || config.GlobalConfig.Terminal.FontSize != 18 ||
...
func TestPersistSettingsDraftRollsBackOnWriteFailure(t *testing.T) {
	...
	draft := settingsDraft{Language: "zh-CN", CommandTimeoutSeconds: 10, TerminalFontSize: 18, ResetWorkspaceOnStart: true, StartWithFirstConnection: true}
	if err := persistSettingsDraft(draft); err == nil {
		t.Fatal("expected persistence failure")
	}
	if config.GlobalConfig.Language != "en" || config.GlobalConfig.Terminal.CommandTimeoutSeconds != 60 || config.GlobalConfig.Terminal.FontSize != 14 ||
		config.GlobalConfig.ResetWorkspaceOnStart || config.GlobalConfig.StartWithFirstConnection {
